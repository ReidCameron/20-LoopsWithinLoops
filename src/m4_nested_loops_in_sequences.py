...
def run_test_sum_numbers():
    ...
    # Test 3:
    expected = 43
    answer = sum_numbers([(4, 9), (5, -3), [0, 10, 15, 3]])
    print('Expected and actual are:', expected, answer)

    # Test 4:
    expected = 278
    answer = sum_numbers(([255], (-5, 5), [23]))
    print('Expected and actual are:', expected, answer)

    # Test 5:
    expected = 24.7094304
    answer = round(sum_numbers([(math.pi,math.tau), (math.e,math.pi), [math.tau,math.pi]]),8)
    print('Expected and actual are:', expected, answer)
...
def print_characters_slanted(sequence_of_strings):
    ...
    for k in range(len(sequence_of_strings)):
        for i in range(len(sequence_of_strings[k])):
            string = ' '
            print((i * string) + sequence_of_strings[k][i])
